...
static const int lang_table[CLD2::NUM_LANGUAGES] =
{
    ...
    0,                                 // Can't find TG_UNKNOWN_LANGUAGE
    0,                                 // Can't find UNKNOWN_LANGUAGE
    ...
    MAKE_LANG( 'M', 'S', 'A' ),        // Malay (macrolanguage)
    ...
    MAKE_LANG( 'O', 'C', 'I' ),        // Occitan (post 1500)
    MAKE_LANG( 'U', 'R', 'D' ),        // Urdu
    MAKE_LANG( 'B', 'I', 'H' ),        // A hack, Bihari is not present in ISO 639-3, so the code is from 639-2
    ...
    MAKE_LANG( 'I', 'N', 'A' ),        // Interlingua (International Auxiliary Language Association)
    MAKE_LANG( 'K', 'A', 'N' ),        // Kannada
    MAKE_LANG( 'P', 'A', 'N' ),        // Panjabi
    MAKE_LANG( 'G', 'L', 'A' ),        // Scottish Gaelic
    MAKE_LANG( 'S', 'W', 'H' ),        // Swahili (individual language)
    ...
    MAKE_LANG( 'Z', 'H', 'O' ),        // Using Chinese here, not sure if correct
    ...
    MAKE_LANG( 'K', 'I', 'R' ),        // Can't find Kirghiz
    ...
    MAKE_LANG( 'L', 'A', 'O' ),        // Can't find Lao
    ...
    MAKE_LANG( 'S', 'R', 'P' ),        // No Montenegrin, falling back to Serbian
    MAKE_LANG( 'A', 'K', 'A' ),        // Akan
    MAKE_LANG( 'I', 'B', 'O' ),        // Igbo
    0,                                 // Can't find MAURITIAN_CREOLE
    ...
    0,                                 // Can't find LUBA_LULUA
    0,                                 // Can't find LUO_KENYA_AND_TANZANIA
    ...
    MAKE_LANG( 'C', 'R', 'S' ),        // Seselwa Creole French
    MAKE_LANG( 'T', 'U', 'M' ),        // Tumbuka
    MAKE_LANG( 'V', 'E', 'N' ),        // Venda
    MAKE_LANG( 'W', 'A', 'R' ),        // Waray (Philippines)
};


int cld2_detect_language(const char *buffer, int buffer_length, bool *is_reliable)
{
    CLD2::Language lang = CLD2::DetectLanguage( buffer, buffer_length, true,  is_reliable );

    if ( lang >=0 && lang < sizeof(lang_table) / sizeof(int) )
